...
data <- data %>%
  mutate(returns = (close - lag(close)) / lag(close),
         ma_5 = rollapplyr(returns, 5, mean, fill = NA),
         ma_30 = rollapplyr(returns, 30, mean, fill = NA),
         vol_5 = rollapplyr(returns, 5, sd, fill = NA),
         vol_30 = rollapplyr(returns, 30, sd, fill = NA)) %>%
